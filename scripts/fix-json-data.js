...
async function fixJsonData() {
  try {
    ...
    const regretsResponse = JSON.parse(execSync(`appwrite databases list-documents --database-id ${config.databaseId} --collection-id regrets --json`, { encoding: 'utf8' }));
    ...
    for (const regret of regrets) {
      ...
      if (needsUpdate) {
        try {
          const updateDataString = JSON.stringify(updateData).replace(/"/g, '\\"');
          execSync(`appwrite databases update-document --database-id ${config.databaseId} --collection-id regrets --document-id ${regret.$id} --data '${updateDataString}'`, { stdio: 'inherit' });
          updatedCount++;
        } catch (error) {
          console.error(`❌ Failed to update regret ${regret.$id}:`, error.message);
        }
      }
    }
    ...
    const commentsResponse = JSON.parse(execSync(`appwrite databases list-documents --database-id ${config.databaseId} --collection-id comments --json`, { encoding: 'utf8' }));
    ...
    for (const comment of comments) {
      ...
      if (needsUpdate) {
        try {
          const updateDataString = JSON.stringify(updateData).replace(/"/g, '\\"');
          execSync(`appwrite databases update-document --database-id ${config.databaseId} --collection-id comments --document-id ${comment.$id} --data '${updateDataString}'`, { stdio: 'inherit' });
          updatedCount++;
        } catch (error) {
    ...
    console.log(`✅ JSON data fix completed! Updated ${updatedCount} documents.`);
  } catch (error) {
