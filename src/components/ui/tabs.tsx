...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "bg-muted/50 text-muted-foreground inline-flex items-center justify-center rounded-lg p-1 backdrop-blur-sm space-x-2",
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ...
    className={cn(
      // Base styles
      "inline-flex items-center justify-center text-sm font-medium whitespace-nowrap transition-all duration-200 cursor-pointer",
      // Sizing and spacing
      "rounded-md px-3 py-1.5",
      // Default state
      "text-muted-foreground hover:text-foreground",
      "hover:bg-muted/60 hover:shadow-sm",
      // Active state - enhanced with proper contrast
